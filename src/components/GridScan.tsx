...
type GridScanProps = {
  ...
  forwardRef?: React.MutableRefObject<number>;
  forwardEase?: number;
  pointerWindow?: boolean;
};
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  float lineMask = max(primaryMask, altMask);
    ...
    float scanZMax = max(0.5, uScanDepth);
    float localZ = hit.z - ro.z;
...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    const moveTarget: Window | HTMLElement = pointerWindow ? window : el;
    moveTarget.addEventListener("mousemove", onMove as EventListener);
    if (scanOnClick) moveTarget.addEventListener("click", onClick as EventListener);
    if (!pointerWindow) {
    ...
    return () => {
      ...
      if (scanOnClick) moveTarget.removeEventListener("click", onClick as EventListener);
      if (!pointerWindow) {
  ...
  }, [uiFaceActive, snapBackDelay, scanOnClick, enableGyro, pointerWindow]);
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uScanCount: { value: 0 },
      uForward: { value: forwardCurrent.current },
    ...
    const tick = () => {
      ...
      const forwardTarget = forwardRef?.current ?? 0;
      forwardCurrent.current +=
        (forwardTarget - forwardCurrent.current) * forwardEase;
      material.uniforms.uForward.value = forwardCurrent.current;
  ...
  useEffect(() => {
    ...
    if (m) {
      ...
      u.uScanDepth.value = Math.max(0.5, scanDepth);
    }
    if (bloomRef.current) {
